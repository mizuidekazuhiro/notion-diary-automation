...
def fetch_prompt_from_notion(notion: NotionClient, cfg: Config, prompt_type: str) -> str | None:
    ...
    LOGGER.info(
        "prompt_fetch: type=%s candidates=%s target=%s language=%s time_style=%s selection=approved_and_active priority_desc",
        prompt_type,
        len(pages),
        cfg.prompt_target,
        cfg.prompt_language,
...
def generate_diary_from_gpt(payload: dict[str, Any], api_key: str, model: str, notion: NotionClient, cfg: Config) -> str:
    ...
    LOGGER.info(
        "prompt_db: enabled=%s target=%s variant=%s variant_filter_used=false time_style=%s",
