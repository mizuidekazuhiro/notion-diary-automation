...
def main() -> None:
    ...
    assert_contains(
        r'property:\s*"Status"\s*,\s*select:\s*\{\s*equals:\s*doneStatus',
        index_text,
        "Done filter should include Status == doneStatus.",
    )
    assert_contains(
        r'property:\s*"Status"\s*,\s*select:\s*\{\s*equals:\s*droppedStatus',
        index_text,
        "Drop filter should include Status == droppedStatus.",
    )
    assert_contains(
        r'property:\s*"Done date"\s*,\s*date:\s*\{\s*is_not_empty:\s*true',
        index_text,
        "Done filter should require Done date is_not_empty.",
    )
    assert_contains(
        r'property:\s*"Drop date"\s*,\s*date:\s*\{\s*is_not_empty:\s*true',
        index_text,
        "Drop filter should require Drop date is_not_empty.",
    )
    assert_contains(
        r'property:\s*"Done date"[\s\S]*on_or_after:[\s\S]*before:',
        index_text,
        "Done filter should include date range.",
    )
    assert_contains(
        r'property:\s*"Drop date"[\s\S]*on_or_after:[\s\S]*before:',
        index_text,
