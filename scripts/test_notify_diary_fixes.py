...
def test_expense_f_created_time_fallback_and_no_category(monkeypatch) -> None:
    ...
    assert result.debug_summary["resolved_props"]["date"]["resolved_name"] == "Date"
    assert result.debug_summary["resolved_props"]["category"]["resolved_name"] in {None, "Category"}
    assert calls
    and_terms = calls[0]["filter"]["and"]
    assert and_terms[0] == {"property": "F", "checkbox": {"equals": True}}
    assert and_terms[1]["timestamp"] == "created_time"
    assert and_terms[2]["timestamp"] == "created_time"
